//
//  BilgiHavuzuTableViewController.swift
//  AdanaBaro
//
...
import UIKit
import SWXMLHash
class BilgiHavuzuTableViewController: UITableViewController {

    var poolmodel = [informationpool]()
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return poolmodel.count
    }

   
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "bilgiHavuzuCell", for: indexPath) as! BilgiHavuzuTableViewCell

        // Configure the cell...
        cell.image_name.Load(url: poolmodel[indexPath.row].image)
        cell.lbl_bilgi.text=poolmodel[indexPath.row].title
       cell.poolview.layer.cornerRadius=7
        cell.poolview.layer.borderWidth=2
        cell.poolview.layer.borderColor=UIColor.red.cgColor
       // cell.poolview.backgroundColor = UIColor(patternImage: UIImage(named: "BUTTONYAPISI_3.png")!)
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        print("tıklandı \(poolmodel[indexPath.row].Id)")
      /*  yayinid = yayinlarmodel[indexPath.row].Id*/
        informayionpool_Id = poolmodel[indexPath.row].Id
              let storyboard :UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
            
              let vc = storyboard.instantiateViewController(withIdentifier: "bilgidetayStoryboard") as! BilgiHavuzuDetayViewController
                vc.informayionpool_Id = informayionpool_Id
     ...
     override func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
       ...
       override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
           let headerView = Bundle.main.loadNibNamed("BaroHeaderTableViewCell", owner: self, options: nil)?.first as? BaroHeaderTableViewCell;
                  ...
                  headerView?.lbl_baslık.text = "BİLGİ HAVUZU";
    ...
    func information_pool(){
    print("fonksiyon içi")
         let urlString = "\(API_URL)/get_information_pool.php"
                           ...
                           //  AppDelegate.showAlertError(AppDelegate.getConnectionErrorMessage())
                            ...
                            for i in 0..<Int(cardview!.count){
                               ...
                               let cardview1 = cardview?[i].components(separatedBy: "[#]")
                                let poollist = informationpool()
                                poollist.Id=String((cardview1?[0])!)
                                poollist.title = String((cardview1?[1])!)
                                poollist.image = String((cardview1?[2])!)
                               

                                self.poolmodel.append(poollist);
                                
                                
                            }
                           print("dizi içi\(self.poolmodel[0].title)")
                             print("dizi içi1\(self.poolmodel[1].title)")
